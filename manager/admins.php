...
if (isset($_POST['submit'])) {
 ...
 elseif($password !=$passwordcom){
  ...
  $stmt-> bind_param('s',$em_email,$adminemail);
  $stmt->execute();
  $result = $stmt->get_result();
  $count =$result->num_rows;
  $stmt->close();
  if($count>0){
    $error='<div class="btn btn-danger">Email already exist!</div>';
  }else{
    
    $password = password_hash($password, PASSWORD_DEFAULT);
    $sql ="INSERT INTO employees(em_name,em_email,adminemail,em_number,em_role,b_id,password)VALUES(?,?,?,?,?,?,?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('sssssss',$em_name,$em_email,$adminemail,$em_number,$em_role,$b_id,$password);
    if ($stmt->execute()){
